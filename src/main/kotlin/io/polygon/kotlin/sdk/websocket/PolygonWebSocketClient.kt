...
constructor(
    ...
    private fun processFrameJson(
        ...
        if (frame is JsonObject) {
            val message = when (frame.jsonObject.get(EVENT_TYPE_MESSAGE_KEY)?.jsonPrimitive?.contentOrNull) {
    ...
    private fun JsonElement.isStatusMessage() =
        ...
        this is JsonObject && jsonObject.get(EVENT_TYPE_MESSAGE_KEY)?.jsonPrimitive?.contentOrNull == "status"
}
